...
for i in range(1, 11):
    ...
    while running:
        ...
        print(f'Iteration {i}')
        print(f'A* Desired user goal: {user_goal_a_star}')
        print(f'A* Actual particle position: {particle_pos_a_star}')
        print(f'A* Actual velocity: {direction_a_star}')
        print(f'BFS Desired user goal: {user_goal_bfs}')
        print(f'BFS Actual particle position: {particle_pos_bfs}')
        print(f'BFS Actual velocity: {direction_bfs}')
        print(f'RRT Desired user goal: {user_goal_rrt}')
        print(f'RRT Actual particle position: {particle_pos_rrt}')
        print(f'RRT Actual velocity: {direction_rrt}')
        ...
        pygame.draw.circle(screen, YELLOW, particle_pos_rrt.astype(int), 5)
        ...
        for j in range(1, len(trajectory_rrt)):
            pygame.draw.line(screen, YELLOW, trajectory_rrt[j-1], trajectory_rrt[j], 2)
    ...
    df_a_star = pd.DataFrame(data_a_star)
    a_star_file_name = f"/Users/yuanzhengsun/Desktop/CBF_sim/CBF/A_star_path/A_star_path_{i}.csv"
    df_a_star.to_csv(a_star_file_name, index=False)
    print(f"A* Data saved to {a_star_file_name}")

    df_bfs = pd.DataFrame(data_bfs)
    bfs_file_name = f"/Users/yuanzhengsun/Desktop/CBF_sim/CBF/bfs_path/bfs_path_{i}.csv"
    df_bfs.to_csv(bfs_file_name, index=False)
    print(f"BFS Data saved to {bfs_file_name}")

    df_rrt = pd.DataFrame(data_rrt)
    rrt_file_name = f"/Users/yuanzhengsun/Desktop/CBF_sim/CBF/rrt_path/rrt_path_{i}.csv"
    df_rrt.to_csv(rrt_file_name, index=False)
    print(f"RRT Data saved to {rrt_file_name}")
